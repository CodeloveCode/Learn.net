...
namespace 读取目录下所有文件路径_包括子目录_
{
    ...
    /// 获取给定目录的文件路径,包括子目录
    /// </summary>
    public static class Path获取给定目录的文件路径
    {
        public static List<string> Path获取目录的文件路径(string 给定的目录)
        {

            //声明一个string类型泛型集合,用来存储文件路径
            List<String> path目录下所有文件 = new List<string>();
            //声明一个字符串数组来 存储目录下 所有子目录路径信息
            string[] path子目录 = Directory.GetDirectories(给定的目录);
            //将当前目录下文件添加到集合里
            path目录下所有文件.AddRange(Directory.GetFiles(给定的目录));
